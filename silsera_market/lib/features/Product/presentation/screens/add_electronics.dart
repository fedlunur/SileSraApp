...
class AddElectronics extends StatefulWidget {
  ...
  const AddElectronics({super.key, required this.city, required this.catagory});

  @override
  _PropertyStepperState createState() => _PropertyStepperState();
}

class _PropertyStepperState extends State<AddElectronics> {
  final electronicsService = ListingProvider().electronicsService;
  int _currentStep = 0;
  ...
  Future productCreate(BuildContext context) async {
    bool isSuccess = await PostService(
      ...
      fromDict: ElectronicsModel.from_dict,
      formData: {
        'name': name,
        'electronicsType': selectedElectronicsType,
    ...
    switch (_currentStep) {
      case 0:
        if (_formKey.currentState?.validate() ?? false) {
          // if (name.isEmpty || price == null || description.isEmpty || selectedElectronicsType == null) {
          ...
          // Conditional validations based on electronics type
          if (selectedElectronicsType == "Mobile Phones" ||
              selectedElectronicsType == "Laptops" ||
              selectedElectronicsType == "Tablets") {
            if (brand == null || model == null) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(
                        "Brand and Model are required for this electronics type.")),
              );
              return false;
            }
          }

          if (selectedCondition == null && selectedElectronicsType != "Other") {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content:
                      Text("Condition is required for this electronics type.")),
            );
            return false;
          }
    ...
    return Form(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 10),
          _buildElectronicsTypeSelection(),
          SizedBox(height: 10),
          _buildTextField("Brand",
              isOptional: selectedElectronicsType != "Mobile Phones" &&
                  selectedElectronicsType != "Laptops" &&
                  selectedElectronicsType != "Tablets"),
  ...
  Widget _buildConditionSelection() {
    return DropdownButtonFormField(
      ...
      validator: (value) => value == null && selectedElectronicsType != "Other"
  ...
  Widget _buildWarrantySelection() {
    return DropdownButtonFormField(
      dropdownColor: Colors.white,
      decoration: _inputDecoration("Warranty Available?"),
      items: warrantyOptions.map((String warranty) {
        return DropdownMenuItem(value: warranty, child: Text(warranty));
      }).toList(),
      onChanged: (value) => setState(() => selectedWarranty = value),
      icon: Icon(Icons.arrow_drop_down, color: Colors.black),
      style: TextStyle(color: Colors.black),
    );
  }

  Widget _buildElectronicsTypeSelection() {
    return DropdownButtonFormField(
      dropdownColor: Colors.white,
      decoration: _inputDecoration("Select Electronics Type"),
      items: electronicsTypes.map((String type) {
        return DropdownMenuItem(value: type, child: Text(type));
      }).toList(),
      onChanged: (value) => setState(() => selectedElectronicsType = value),
      validator: (value) =>
          value == null ? 'Please select an electronics type' : null,
      icon: Icon(Icons.arrow_drop_down, color: Colors.black),
      style: TextStyle(color: Colors.black),
    );
  }
